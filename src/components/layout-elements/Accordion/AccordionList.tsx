      ...
      >
        {React.Children.map(children, (child, idx) => {
          if (idx === 0) {
            return (
              <RootStylesContext.Provider
                value={bellhopTwMerge("rounded-t-bellhop-default border")}
              >
          ...
          if (idx === numChildren - 1) {
            return (
              <RootStylesContext.Provider
                value={bellhopTwMerge(
                  "rounded-b-bellhop-default border-l border-r border-b",
                )}
              >
                {React.cloneElement(child)}
              </RootStylesContext.Provider>
          ...
          return (
            <RootStylesContext.Provider
              value={bellhopTwMerge("border-l border-r border-b")}
            >
              {React.cloneElement(child)}
            </RootStylesContext.Provider>
          );
