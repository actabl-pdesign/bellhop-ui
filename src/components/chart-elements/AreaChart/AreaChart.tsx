...
const AreaChart = React.forwardRef<HTMLDivElement, AreaChartProps>((props, ref) => {
  ...
  return (
    <div ref={ref} className={bellhopTwMerge("w-full h-80", className)} {...other}>
        ...
        {data?.length ? (
          ...
          >
                    ...
                    CustomTooltip ? (
                      <CustomTooltip
                        payload={payload?.map((payloadItem: any) => ({
                          ...payloadItem,
                          color: categoryColors.get(payloadItem.dataKey) ?? BaseColors.Neutral,
                        }))}
                        active={active}
                        label={label}
                      />
            ...
            {categories.map((category) => {
              const gradientId = (categoryColors.get(category) ?? BaseColors.Neutral).replace("#", "");
              return (
                <defs key={category}>
                  {showGradient ? (
                    <linearGradient
