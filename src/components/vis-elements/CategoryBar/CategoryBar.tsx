    ...
    >
      {values.slice(0, values.length).map((widthPercentage, idx) => {
        ...
        return (
          ...
          >
            <span
              className={bellhopTwMerge(showLabel ? "block" : "hidden", "left-1/2 translate-x-1/2")}
            >
              {prefixSum}
            </span>
          </div>
      ...
      <div className={bellhopTwMerge("absolute bottom-0 flex items-center left-0")}>0</div>
      <div className={bellhopTwMerge("absolute bottom-0 flex items-center right-0")}>
        {sumValues}
      </div>
    </div>
  );
...
const CategoryBar = React.forwardRef<HTMLDivElement, CategoryBarProps>((props, ref) => {
  ...
  return (
    <>
      ...
      <div
        ref={ref}
        className={bellhopTwMerge(makeCategoryBarClassName("root"), className)}
        {...other}
      >
